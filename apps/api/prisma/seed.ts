...
async function main() {
  ...
  await prisma.siteSetting.upsert({ where: { key: "site" }, update: {}, create: { key: "site", value: {
    ...
    heroSubtitle: "Каталог готовых работ, галерея выполненных изделий и индивидуальные заказы напрямую у мастера.",
    guaranteeText: "Каждое изделие проходит проверку качества. Документы предоставляются при наличии и по запросу.",
    deliveryText: "Доставка обсуждается индивидуально после оформления заказа.",
    cartDeliveryText: "Сейчас онлайн оплачивается только изделие. Способ, срок и стоимость доставки мастер согласует отдельно после оплаты заказа."
  } } });
